...
function(input, output) {
  ...
  #   
  #   for(i in 1:10){
  #   range_points <- reactive({
  #     subset(plot_points(), plot_points()$Date >= as.Date(as.POSIXct((16225 + 90*(i-1)), origin="1970-01-01"))) #&
  #     #plot_points()$Date <= (16225 + 90*(i+1)))
  #   })
  #   
  #   line_point <- reactive({
  #     range_points()/input$points
  #   })
  #   
  #   
  #   
  ...
  #   return(range_points())
  #   }
                         #[order(plot_points()$Date)]
    #min(plot_points()$Date)
    #x_points <- plot_points()$Date
    #points <- data.table()
  #})
  ...
  output$similarity_plot <- renderPlotly({
    gg=plot_ly(x = plot_points()$Date) %>%
      ...
      add_trace(x = plot_points()$Date[input$similarity_table_rows_selected], 
                ...
                #size = 1 #,
                # text = ~paste("Rank:", plot_points()$Rank[input$similarity_table_rows_selected], 
                #               "<br>Member HoC/HoL:", plot_points()$Question_MP[input$similarity_table_rows_selected],
                #               "<br>Date:", plot_points()$Date[input$similarity_table_rows_selected]),
                hoverinfo = NULL
                ) %>%
