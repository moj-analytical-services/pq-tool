...
if(opt$input_file == 's3') {
    aPQ <- s3readRDS(bucket = 'parliamentary-questions-tool', object = 'raw_archived_questions.rds')
  } else {
    file <- opt$input_file
...
if(save_location == 's3') {
  ...
  bucket = 'parliamentary-questions-tool'
    ...
    s3saveRDS(
      savedf,
      bucket = bucket,
      object = str_interp("processed_questions-${Sys.Date()}.rds")
    )
    s3saveRDS(
