...
if( opt$environment == 'test' ) {
  opt$input_file <- str_interp("${s3_bucket}/tests/testthat/examples/data/lsa_training_sample.csv")
  opt$output_dir <- str_interp("${s3_bucket}/tests/testthat/examples/data")
  opt$k_clusters <- 100
  opt$x_dims <- 100
} else if( opt$environment == 'prod' ) {
  opt$input_file <- str_interp("${s3_bucket}/Data/archived_pqs.csv")
  opt$output_dir <- str_interp("${s3_bucket}/Data")
  opt$k_clusters <- 1000
  opt$x_dims <- 2000
}
...
s3tools::write_df_to_csv_in_s3(savedf, "alpha-pq-tool-data/Data/MoJwrittenPQs.csv", overwrite =TRUE)


#The information about the clusters
#write.csv(topDozenWordsPerTopic, "topDozenWordsPerTopic.csv")
s3tools::write_df_to_csv_in_s3(topDozenWordsPerTopic, "alpha-pq-tool-data/Data/topDozenWordsPerTopic.csv", overwrite =TRUE)

#The information about the members
#write.csv(topDozenWordsPerMember, "topDozenWordsPerMember.csv")
s3tools::write_df_to_csv_in_s3(topDozenWordsPerMember, "alpha-pq-tool-data/Data/topDozenWordsPerMember.csv", overwrite =TRUE)
